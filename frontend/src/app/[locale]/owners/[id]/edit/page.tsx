...
export default function EditOwnerPage({
  ...
  const [owner, setOwner] = useState<Owner>({
    id: id ?? 0, // Use nullish coalescing to provide a default value
    name: "",
    telephone: "",
    email: "",
  ...
  useEffect(() => {
    const fetchOwner = async () => {
      ...
      try {
        setIsLoading(true);
        const data = await ownerService.getById(id, locale.toString());
        setOwner(data);
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await ownerService.update(id, owner, locale.toString());
      router.push(`/${locale}/owners`);
      router.refresh();
    } catch (error) {
